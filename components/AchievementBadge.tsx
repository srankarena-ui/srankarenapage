...
export default function AchievementBadge({ achievement, userValue }: { achievement: any, userValue: number }) {
  ...
  // Encontrar el valor del siguiente tier
  ...
  return (
    <div className={`relative p-6 rounded-[2rem] border-2 transition-all duration-500 ${tierStyle}`}>
      <div className="flex items-center gap-5">
        {/* Icono de Tier */}
        <div className={`w-14 h-14 rounded-full flex items-center justify-center font-black italic text-xl border-2 shadow-inner ${currentTier === 'locked' ? 'border-gray-800' : 'border-current'}`}>
        ...
        <div className="flex-1">
          <div className="flex justify-between items-end mb-1">
            ...
            <span className="text-[9px] font-bold opacity-60 uppercase">{userValue} / {nextTierValue}</span>
          </div>
      ...
      {currentTier === 's_rank' && (
        <div className="absolute -top-2 -right-2 bg-[#A855F7] text-white text-[8px] font-black px-3 py-1 rounded-full uppercase tracking-tighter shadow-lg">
          Mastered
        </div>
      )}
    </div>
